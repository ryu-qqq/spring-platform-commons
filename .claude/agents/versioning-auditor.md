...
| `binary-compat-gate` | 배포 모듈 빌드/CI에 바이너리 호환 게이트(`japicmp`/`revapi` 플러그인) 존재. **단 루트 version이 pre-1.0(0.x)이면 `info`(ADR-0004로 1.0까지 의도적 defer — 0.x는 minor breaking 허용)** | 1.0 이상인데 게이트 부재 → fail |
