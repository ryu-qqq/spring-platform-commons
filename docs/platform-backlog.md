...
domain → application → adapter-in / adapter-out → bootstrap
         ↑ platform-common-*     ↑ platform-persistence-jpa, resilient-client, platform-outbox-*, …
```
...
| `platform-persistence-jpa` | adapter-out | `BaseAuditEntity`, soft delete, `@Version`, `JPAQueryFactory`, QueryDSL Q-types | 2 ✅ |
...
1. 같은 Aggregate / Outbox 행 경쟁?
   → @Version 낙관적 락 (platform-persistence-jpa + platform-web 409)
...
| ID | 산출 | 상태 |
|----|------|------|
| P2-1 | `platform-persistence-jpa` v0.1 — entity bases, auditing, `JPAQueryFactory` | ✅ |
| P2-2 | `OptimisticLockingFailureException` → platform-web **409** `OPTIMISTIC_LOCK_CONFLICT` | ✅ |
| P2-3 | template `adapter-out/persistence-mysql/example-persistence` (HT6 pattern) | ✅ |
| P2-4 | `docs/sdk/platform-persistence-jpa.md`, README, backlog | ✅ |

**비목표 (Phase 2):** per-domain QueryDSL `ConditionBuilder` SDK, `AuditorAware`/createdBy, Manager 1회 재시도 코드화 (wiki 가이드만).
...
| Phase 2 persistence | "platform-persistence-jpa + 409 + example-persistence — Manager retry는 문서만" |
...
| P2-1 | platform-persistence-jpa v0.1 | https://www.notion.so/36be8135530581a8a658c2ee60e768cf |
...
| 2026-05-27 | **Phase 2 완료** — platform-persistence-jpa, optimistic lock 409, example-persistence, SDK docs |
