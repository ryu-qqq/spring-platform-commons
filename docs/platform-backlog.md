...
**비목표 (Phase 2):** template `example-persistence` 모듈 (wiki/docs로 배선 가이드), per-domain QueryDSL `ConditionBuilder` SDK, `AuditorAware`/createdBy, Manager 1회 재시도 코드화 (wiki 가이드만).
...
| Phase 2 persistence | "platform-persistence-jpa + 409 user message — example 모듈 없음, wiki 배선" |
...
| 2026-05-27 | **Phase 2 완료** — platform-persistence-jpa, optimistic lock 409, SDK docs (example-persistence 제외) |
