...
| Hexagonal Gradle 골격 (HT1) | 🔄 로컬 구현됨 — `domain` / `application` / `adapter-in:rest-api` / `bootstrap:bootstrap-web-api` |
...
| `platform-bootstrap-web` | bootstrap | JSON logging, actuator, Sentry, MDC | 1 |
| `platform-persistence` | adapter-out | `BaseEntity`(`@Version`), QueryDSL condition helpers, Auditing | 2 |
| `resilient-client` | adapter-out | HTTP client CB/retry/timeout (기존) | 2 (HT6) |
...
| ID | Task | 수용 기준 (요약) | ingest 단위 |
|----|------|------------------|-------------|
| **HT1** | Gradle hexagonal skeleton | `settings.gradle` 5모듈, wiki deps, `./gradlew build` green | ✅ 1 commit |
| HT2 | `platform-common-domain` v0.1 | Query/paging VOs + `Versioned` | 1 Task |
| HT3 | `platform-web` + bootstrap wiring | ApiResponse, exception handler, HT1 bootstrap `bootRun` | 1 Task |
| HT4 | `platform-bootstrap-web` | logging JSON, actuator, MDC | 1 Task |
| HT5 | wiki-aligned ArchUnit smoke | 레이어 의존 방향 테스트 | 1 Task |
| HT6 | `resilient-client` v2 in template | adapter-out 예시 + timeout (기존 P0-1 통합) | 1 Task |

### Phase 1 — Core + Web + Bootstrap

- `platform-common-domain`, `platform-common-application`, `platform-web`, `platform-bootstrap-web`
...
- `resilient-client` v0.2 timeout + JitPack publish (아래 Legacy P0)
...
| ID | Task | Phase | 비고 |
|----|------|-------|------|
| P0-1 | resilient-client v0.2 — timeout | 2 / HT6 | [Notion](https://www.notion.so/36be813553058195a061c873cbcdc71c) |
...
| HT4 | platform-bootstrap-web v0.1 | https://www.notion.so/36be8135530581018e2ecb33636f0367 |
