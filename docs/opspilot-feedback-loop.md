...
git log -1 --format=%s   # subject — ingest 게이트 확인
```

### 커밋 subject (필수 — v0.6.0)

`.claude/project.yaml` → `git.commit.requireOnIngest: true`

```
<type>(<scope>): SPC-<번호> <한 줄 요약>
```

예: `docs(platform): SPC-120 Atlantis gap 문서 보강`  
규격: `.claude/references/conventions/commit-format.md` · Cursor: `.cursor/rules/commit-format-spc.mdc`

형식이 맞지 않으면 ingest API가 `InvalidCommitSubject` 로 거부한다.
...
| 방식 | 언제 | evalSource |
|------|------|------------|
| **자동** | `engineering-os` **`eo-done`** — Task 완료 처리 시 fixture ingest (best-effort) | `fixture` |
| **수동** | Cursor `/opspilot-ingest-fixture` · `/opspilot-ingest-session` | `fixture` / `local-claude` |

> **push·commit만으로는 ingest가 돌지 않는다.** API·커맨드·`eo-done` 중 하나로 명시적으로 트리거해야 한다.

---

## Step 3 — Ingest (트리거)

피드백 탭 UI에는 ingest **생성 폼이 없다**. API·Cursor 커맨드·`eo-done` 또는 MCP로 트리거한다.
...
*최종 갱신: 2026-05-26 — eo-done 자동 fixture ingest·Cursor 커맨드 연동*
